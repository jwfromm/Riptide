...
def imagefolder_dataset(root, batch_size, split='train', preprocess=None, num_workers=4):
    ...
    imagenet_ds = imagenet_ds.apply(tf.contrib.data.map_and_batch(
    map_func=decode_fn, batch_size=batch_size, num_parallel_batches=num_workers))
    if split == 'train':
        imagenet_ds = imagenet_ds.shuffle(buffer_size=2*batch_size)
        imagenet_ds = imagenet_ds.prefetch(batch_size)
    return imagenet_ds
